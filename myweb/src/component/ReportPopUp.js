...
const ReportPopUp = ({handleLoading, id, handleShowReport }) => {
  ...
  const handleReport = () => {
    const process = async () => {
      try {
        ...
        console.info(id);
        handleLoading(true);
        let res = await authApi().post(endpoints[`report`](id), formData);
        if (res.status === 201) {
          toast.success("CreateReport Successful");
          handleShowReport();
          handleLoading(false);
        }
      } catch (error) {
        handleLoading(false);
        toast.error("You are reported");
        handleShowReport();
      }
